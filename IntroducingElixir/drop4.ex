...
"""
	...
	def handle_drops do
		receive do
			{from, planemo, distance} -> send(from, {planemo, distance, fall_velocity(planemo, distance)})
			handle_drops
		end
  	end
	...
	def fall_velocity(planemo, distance) when distance >= 0 do
		...
		:math.sqrt(2 * PlanemoRecord.planemo(p, :gravity) * distance)
	end
	...
	def insert_into_table([{name, gravity, diameter, distance} | tail]) do
		:ets.insert(:planemos, PlanemoRecord.planemo(name: name,  gravity: gravity, diameter: diameter, distance_from_sun: distance))
